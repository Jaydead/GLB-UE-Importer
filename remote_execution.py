...
class RemoteExecutionConfig(object):
    '''
    Configuration data for establishing a remote connection with a UE instance running Python.
    '''
    def __init__(self):
...
class RemoteExecution(object):
    '''
    A remote execution session. This class can discover remote "nodes" (UE instances running Python), and allow you to open a command channel to a particular instance.
    ...
    def remote_nodes(self):
        '''
        Get the current set of discovered remote "nodes" (UE instances running Python).
    ...
    def start(self):
        '''
        Start the remote execution session. This will begin the discovey process for remote "nodes" (UE instances running Python).
        '''
    ...
    def stop(self):
        '''
        Stop the remote execution session. This will end the discovey process for remote "nodes" (UE instances running Python), and close any open command connection.
        '''
    ...
    def open_command_connection(self, remote_node_id):
        '''
        Open a command connection to the given remote "node" (a UE instance running Python), closing any command connection that may currently be open.
...
class _RemoteExecutionNode(object):
    '''
    A discovered remote "node" (aka, a UE instance running Python).
...
class _RemoteExecutionBroadcastNodes(object):
    '''
    A thread-safe set of remote execution "nodes" (UE instances running Python).
    '''
    def __init__(self):
        self._remote_nodes = {}
...
class _RemoteExecutionBroadcastConnection(object):
    ...
    def open(self):
        '''
        Open the UDP based messaging and discovery connection. This will begin the discovey process for remote "nodes" (UE instances running Python).
        '''
        self._running = True
    ...
    def close(self):
        '''
        Close the UDP based messaging and discovery connection. This will end the discovey process for remote "nodes" (UE instances running Python).
        '''
        self._running = False
...
class _RemoteExecutionCommandConnection(object):
    ...
    Args:
        config (RemoteExecutionConfig): Configuration controlling the connection settings.
        node_id (string): The ID of the local "node" (this session).
        remote_node_id (string): The ID of the remote "node" (the UE instance running Python).
    '''
